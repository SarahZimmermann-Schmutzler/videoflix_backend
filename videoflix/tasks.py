...
def convert_1080p(source):
    ...
    cmd = 'sudo ffmpeg -i "{}" -s hd1080 -c:v libx264 -crf 23 -c:a aac -strict -2 "{}"'.format(source, converted_file_name)
...
def convert_720p(source):
    ...
    cmd = 'sudo ffmpeg -i "{}" -s hd720 -c:v libx264 -crf 23 -c:a aac -strict -2 "{}"'.format(source, converted_file_name)
    print(source_without_ending,converted_file_name,cmd)
    subprocess.run(cmd, shell=True, capture_output=True)
...
def convert_480p(source):
    ...
    cmd = 'sudo ffmpeg -i "{}" -s hd480 -c:v libx264 -crf 23 -c:a aac -strict -2 "{}"'.format(source, converted_file_name)
    print(source_without_ending,converted_file_name,cmd)
    subprocess.run(cmd, shell=True, capture_output=True)